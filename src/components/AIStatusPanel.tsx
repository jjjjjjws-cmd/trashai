  ...
  return (
    <div className={`px-3 py-2.5 rounded-lg border ${config.bgColor} ${config.borderColor} animate-in fade-in slide-in-from-left-1 duration-300`}>
      <div className="flex items-start gap-2">
        <span className={`text-[11px] font-mono font-bold mt-0.5 flex-shrink-0 ${config.color}`}>{config.icon}</span>
        <div className="min-w-0">
          <p className={`text-xs font-semibold ${config.color}`}>{concept.term}</p>
          <p className="text-[11px] text-stone-500 mt-0.5 leading-snug">{concept.description}</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] font-mono uppercase tracking-widest text-stone-400">{title}</span>
        <div className="flex-1 h-px bg-stone-200" />
        <span className={`text-[10px] font-mono ${config.color}`}>{concepts.length}</span>
      </div>
      ...
      `}>
        ...
        <div className="px-4 py-4 border-b border-stone-100">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xs font-semibold text-stone-700 tracking-tight">AI 이해 상태</h2>
            ...
            <div className="flex flex-col items-end gap-1">
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] font-mono text-stone-400">이해도</span>
                <span className="text-sm font-bold font-mono text-amber-600">{state.comprehensionScore}%</span>
              </div>
              <div className="w-20 h-1.5 bg-stone-100 rounded-full overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-red-400 via-amber-400 to-emerald-500 rounded-full transition-all duration-700 ease-out"
                  style={{ width: `${state.comprehensionScore}%` }}
                />
              </div>
            </div>
          </div>
        ...
        {state.currentReasoning && (
          <div className="mx-3 mt-3 p-3 rounded-lg bg-stone-50 border border-stone-200">
            <div className="flex items-center gap-2 mb-2">
              <div className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" />
              <span className="text-[10px] font-mono uppercase tracking-widest text-stone-400">AI 현재 추론</span>
            </div>
            <p className="text-[11px] text-stone-500 leading-relaxed font-mono">{state.currentReasoning}</p>
          </div>
        )}
          ...
          {totalConcepts === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <div className="text-center">
                <p className="text-xs text-stone-500">아직 배운 개념이 없어요</p>
                <p className="text-[10px] text-stone-400 font-mono mt-1">무언가를 가르쳐 보세요</p>
              </div>
            </div>
          ) : (
            <>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <div key={stat.label} className={`${stat.bg} rounded-lg p-2 text-center`}>
                <p className={`text-sm font-bold font-mono ${stat.color}`}>{stat.count}</p>
                <p className={`text-[10px] font-mono ${stat.color} opacity-70`}>{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
